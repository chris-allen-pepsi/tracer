defmodule Tracer.Tool.FlameGraph do
  ...
  """
  alias __MODULE__
  alias Tracer.{Probe, ProcessHelper, EventCall, EventReturnTo, EventIn, EventOut}
  use Tracer.Tool
  ...
  defp handle_event_call(
         %EventCall{mod: m, fun: f, arity: a, ts: ts},
         state
       ) do
    ms_ts = ts_to_ms(ts)
  ...
  defp handle_event_return_to(
         %EventReturnTo{mod: m, fun: f, arity: a, ts: ts},
         state
       ) do
    ms_ts = ts_to_ms(ts)
  ...
  defp report_stack(
         %{stack: stack, stack_acc: [{stack, stack_time} | stack_acc], last_ts: last_ts} = state,
         ts
       ) do
    %{state | stack_acc: [{stack, stack_time + (ts - last_ts)} | stack_acc], last_ts: ts}
  end

  defp report_stack(
         %{stack: stack, stack_acc: stack_acc, last_ts: last_ts} = state,
         ts
       ) do
