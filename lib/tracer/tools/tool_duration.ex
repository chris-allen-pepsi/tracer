defmodule Tracer.Tool.Duration do
  ...
  def handle_event(event, state) do
    case event do
      %EventCall{pid: pid, mod: mod, fun: fun, arity: arity, ts: ts, message: c} ->
        ts_ms = ts_to_ms(ts)
        key = inspect(pid)

        new_stack = [
          {mod, fun, arity, ts_ms, c}
      ...
      %EventReturnFrom{pid: pid, mod: mod, fun: fun, arity: arity, ts: ts} ->
        ...
        case Map.get(state.stacks, key, []) do
          ...
          [{^mod, ^fun, ^arity, _, _}, {^mod, ^fun, ^arity, entry_ts, c} | poped_stack] ->
            put_in(
              state.stacks,
              Map.put(state.stacks, key, [{mod, fun, arity, entry_ts, c} | poped_stack])
            )
  ...
  defp handle_aggregation_if_needed(
         %Duration{aggregation: nil} = state,
         event
       ) do
    report_event(state, event)
    state
  ...
  defp handle_aggregation_if_needed(
         state,
         %Event{mod: mod, fun: fun, arity: arity, message: message, duration: duration}
       ) do
    collect =
      Collect.add_sample(
        state.collect,
