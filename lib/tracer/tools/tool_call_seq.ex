defmodule Tracer.Tool.CallSeq do
  ...
  """
  alias __MODULE__
  alias Tracer.{EventCall, EventReturnFrom, Probe, ToolHelper, Tool.CallSeq.Event, ProcessHelper}
  ...
  @allowed_opts [:ignore_recursion, :max_depth, :show_args, :show_return, :start_match]
  ...
  def init(opts) do
    ...
    init_state =
      case Keyword.get(opts, :start_match) do
        nil ->
          init_state
  ...
  def handle_event_call(
        %EventCall{pid: pid, mod: mod, fun: fun, arity: arity, ts: ts, message: m},
        state
      ) do
  ...
  defp push_to_stack(key, {:enter, _, _} = stack, state) do
    state =
      if Map.get(state.depth, key, 0) < state.max_depth do
        new_stack = [
          stack
  ...
  defp push_to_stack(key, {:exit, _, _} = stack, state) do
    state =
      if Map.get(state.depth, key, 0) < state.max_depth + 1 do
        new_stack = [
          stack
          | Map.get(state.stacks, key, [])
        ]
